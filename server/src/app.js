...
app.post('/api/sessions', async (req, res) => {
  const { date, dayType, exercises = {}, cardio = false, notes = '', completed = false, kcal = 0 } = req.body;
...
app.get('/api/summary', async (_req, res) => {
  try {
    const { rows: sessions } = await db.query('SELECT * FROM workout_sessions WHERE completed = true');
